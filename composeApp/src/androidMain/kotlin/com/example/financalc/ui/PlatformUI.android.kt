...
actual fun CapitalizationSelector(
    ...
    Column(modifier) {
        ...
        // Slider — per requirements (Android variant). Adjusts visual emphasis only.
        var emphasis by remember { mutableStateOf(0.5f) }
        Spacer(Modifier.height(8.dp))
        Slider(value = emphasis, onValueChange = { emphasis = it })
    }
}
...
actual fun GrowthChart(series: List<CalculationResult.Point>, modifier: Modifier) {
    ...
    Canvas(modifier.testTag("growth_chart")) {
        ...
        // Gradient fill — Android requirement.
        drawPath(
            path = fillPath,
            brush = Brush.verticalGradient(
                colors = listOf(primary.copy(alpha = 0.35f), primary.copy(alpha = 0.05f)),
                startY = 0f, endY = h
            )
        )
        drawPath(path = path, color = primary, style = Stroke(width = 4f))
        // axes baseline
        drawLine(onSurface.copy(alpha = 0.4f), Offset(padding, h - padding), Offset(w - padding, h - padding), strokeWidth = 1.5f)
    }
}
